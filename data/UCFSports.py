...
class UCFSportsDetection(data.Dataset):
    ...
    def __init__(self, root = UCFSports_Images_Dir,transform=None, target_transform=None,dataset_name='UCFSports'):
        ...
        for trainlist in d.train_vlist():
            each_video_dir = os.path.join(UCFSports_Images_Dir, trainlist)
            video_frame_num = d.nframes(trainlist)
            labels = list(d._gttubes[trainlist].keys())[0]
            resoliution = d._resolution[trainlist]
            for nframe in range(video_frame_num):
                imgdir = os.path.join(each_video_dir, "{:0>6}".format(nframe + 1) + ".jpg")
                self.ids.append((imgdir, trainlist,nframe,labels,resoliution))
