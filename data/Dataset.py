...
torch_Path = os.path.dirname(__file__)
# ROOT_DATASET_PATH = "/path/to/datasets/"  # CHANGEME
ROOT_DATASET_PATH = "/home/haoliangtan/Desktop/gradution_project/caffe/dataset"
...
class TubeDataset(object):
    ...
    def __init__(self, dname, split=1):
        ...
        cache_file = os.path.join(torch_Path, 'cache', dname + '-GT.pkl')
        assert os.path.isfile(cache_file), "Missing cache file for dataset " + dname

        # with open(cache_file, 'rb') as fid:
        #     cache = pickle.load(fid,encoding='iso-8859-1')
        #     print('test ')
        # # cache = pickle.loads(open(cache_file, "rb"))
        # # cache = pickle.loads()
        try:
            cache = pickle.load(open(cache_file, 'rb'), encoding='iso-8859-1')
        except:
            print('An error occurs in open cache files')
...
class UCFSports(TubeDataset):
   ...
   #      return os.path.join("/home/haoliangtan/Desktop/gradution_project/caffe/dataset", "UCFSports", "Frames", v, "{:0>6}.jpg".format(i))

    def flowfile(self, v, i):
        return os.path.join(ROOT_DATASET_PATH, "UCFSports", "FlowBrox04", v, "{:0>6}.jpg".format(i))
        # return os.path.join("/home/haoliangtan/Desktop/gradution_project/caffe/dataset", "UCFSports", "FlowBrox04", v, "{:0>6}.jpg".format(i))
